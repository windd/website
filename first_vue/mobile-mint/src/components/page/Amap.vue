<template>
  <div>
      ...
      <!--<p v-for="my in station" :key="my.id">{{my.id}}</p>
      <p>{{station}}</p>-->
  </div>
  ...
  methods: {
    ...
    mapinit () {
      ...
      while (i) {
        ...
        label = new BMap.Label(this.station[i - 1].name, {point: point, offset: new BMap.Size(20, 5)})
        ...
        centerW = this.station[i - 1].latitude + centerW
        i--
      }
      // 转点这个点
      point = new BMap.Point(centerJ / this.station.length, centerW / this.station.length)
      map.panTo(point)
